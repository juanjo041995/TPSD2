...
################################################## SKIP TO LINE 736 FOR ACTUAL CODE #######################################################
	la $sp, 0x100101FC # DIRECCION FINAL DE LA MEMORIA DE DATOS
	addi $t1,$zero,0xFF
	addi $t2,$zero,0x00
...
updateScreen:
	...
	colored:						# otherwise, it's colored.
		...
		afterEtiq2:	addi $t9, $t9, 1 #CAMBIADO 4 por 1			# go to next position in array	
			...
			j checkOOB	
		etiquet2:    addi $s7, $zero, 1
			j afterEtiq2		# continue looping through screen.
	...
	black:	
		...
		afterEtiq1: addi $t8, $t8, 1 # 4 por 1			# go to next space in array			
			addi $t7, $t7, 1 
			j checkOOB
					# increment live pixels
		etiquet1:    addi $s6, $zero, 1
			j afterEtiq1
...
nextGeneration:
	...
	DIRECCION:	la $t3, 0xFFFF6000 #Direccion para leer SET
		lw $t5, ($t3)  # VALOR DE SET en $t5
		beq $t5, 0, pause # si set es cero se salga a pause
		
	CARGAR:	la $t3, 0xFFFF8000 # DIRECCION PARA LEER SWITCHES
		lw $t4, ($t3)       #Se carga valor establecido por los SWITCHES
		addi $t4,$t4,0xFFFF0000 #Se suma a la base del array de la pantalla
		
		lbu $t6,($t4) # Se carga valor de la direccion en cuestion
		beq $t6,0x00,EQNEGRO
