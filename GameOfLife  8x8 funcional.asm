...
createPixel:
	...
	sll $t2, $a1, 3 #Corregido direccion por 4 por la pantalla 4x4			# calculates pixel y-position by multiplying y-position by 256, the size of an entire row in this display.
...
checkColor:
	...
	sll $t2, $a3, 3	#cambiado 8 por 2		# calculates neighbor's y-position
...
XYtoAddress:
	...
	sll $t6, $a1, 3 #cambio	2 por 3 para 8x8		# calculates y-position
...
updateScreen:
	...
	checkOOB:
		addi $a0, $a0, 1
		bgt $a0, 7, resetX	#cambiado 63 por 3		# if x-coordinate is over the right of the screen (bound is 63), reset it and increment y-coordinate
		blt $a0, 8, checkPixel			# if x and y-coordinate are within bounds, continue looping through screen
		resetX:
			li $a0, 0
			addi $a1, $a1, 1
			bgt $a1, 7, endUpdateScreen #cambiado 63 por 3	# if y-coordinate reaches the bottom right of the screen, end
			j checkPixel
...
countNeighbors:
	...
	checkOOB3:
		...
		bgt $a2, 7, checkOOB4 #CAMBIADO 63 por 3		# if x-position is OOB on the right side of display, go to next neighbor
		subi $a3, $a1, 1		# get neighbor y-position
	...
	checkOOB5:
		addi $a2, $a0, 1
		bgt $a2, 7, checkOOB6 #CAMBIADO 63 por 3
		move $a3, $a1
		# if not OOB, check if this cell is alive or dead
		jal checkColor
		sne $t6, $v0, 0 			
	...
	checkOOB6:
		...
		blt $a2, 0, checkOOB7
		addi $a3, $a1, 1
		bgt $a3, 7, checkOOB7	#CAMBIADO 63 por 3	# if neighbor's y-position is OOB on the bottom of display, go to next neighbor
		# if not OOB, check if this cell is alive or dead
		jal checkColor
		sne $t6, $v0, 0 			
		add $t5, $t5, $t6
	...
	checkOOB7:
		move $a2, $a0
		addi $a3, $a1, 1
		bgt $a3, 7, checkOOB8 #CAMBIADO 63 por 3
		# if not OOB, check if this cell is alive or dead
		jal checkColor
		sne $t6, $v0, 0 			
		add $t5, $t5, $t6
	...
	checkOOB8:
		addi $a2, $a0, 1
		bgt $a2, 7, endCheck #CAMBIADO 63 por 3
		addi $a3, $a1, 1
		bgt $a3, 7, endCheck #CAMBIADO 63 por 3
		# if not OOB, check if this cell is alive or dead
		jal checkColor
		sne $t6, $v0, 0 			
		add $t5, $t5, $t6
